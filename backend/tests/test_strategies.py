...
def _intraday_ma60_bars() -> list[IntradayBar]:
    ...
    bars.append(_intraday_bar(64, 100.82, 280_000))
    return bars


def _intraday_ma60_bars_below_zero_macd() -> list[IntradayBar]:
    bars = [_intraday_bar(index, 110 - index * 0.08) for index in range(65)]
    return bars
...
def test_intraday_ma60_touch_finds_hourly_ma_support() -> None:
    ...
    assert signal.metrics["daily_ma20_rising"] is True
    assert signal.metrics["daily_ma60_rising"] is True
    assert signal.metrics["daily_ma20_ma60_rising"] is True
    assert signal.metrics["daily_ma20_slope_percent"] > 0
    assert signal.metrics["daily_ma60_slope_percent"] > 0
...
def test_intraday_ma60_touch_rejects_declining_daily_mas() -> None:
...
def test_intraday_ma60_touch_rejects_intraday_macd_below_zero() -> None:
    assert (
        intraday_ma60_touch_signal(
            _uptrend_base(),
            _intraday_ma60_bars_below_zero_macd(),
        )
        is None
    )
